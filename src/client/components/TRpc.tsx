...
export const TRpc: React.FC = () => {
  ...
  const { data: myUser } = trpc.users.getUserById.useQuery(myUserId);
  const { data: allUsers } = trpc.users.getAllUser.useQuery();

  const userCreator = trpc.users.createUser.useMutation({
    async onSuccess() {
      await queryClient.invalidateQueries({
        queryKey: getQueryKey(trpc.users.getAllUser),
      });
    },
  });
  ...
  trpc.users.randomNumber.useSubscription(undefined, {
    onData(data) {
